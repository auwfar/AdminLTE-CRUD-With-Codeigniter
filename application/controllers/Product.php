...
class Product extends AUTH_Controller
{
	...
	public function detail()
	{
		...
		$id 				= trim($_POST['id']);
		$data['product'] = $this->M_product->select_by_id($id);
		$data['dataProduct'] = $this->M_product->select_by_pegawai($id);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if (!$this->upload->do_upload('excel')) {
				...
				foreach ($sheetData as $key => $value) {
					if ($key != 1) {
						$check = $this->M_produc->check_nama($value['B']);
